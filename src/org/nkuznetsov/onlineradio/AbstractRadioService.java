...
public abstract class AbstractRadioService extends Service implements OnErrorListener, OnCompletionListener
{
	...
	protected abstract String wrapStream(String stream);
		...
		public void run() 
		{
			...
			while (mediaPlayer == null && r <= 10)
			{
				try
				{
					...
					MediaPlayer mp = MediaPlayer.create(getApplicationContext(), Uri.parse(wrapStream(url)));
					if (time == timestamp && STATE == STATE_PREPARING)
					{
						mediaPlayer = mp;
						mediaPlayer.setAudioStreamType(AudioManager.STREAM_MUSIC);
						mediaPlayer.setOnErrorListener(AbstractRadioService.this);
						mediaPlayer.setOnCompletionListener(AbstractRadioService.this);
						mediaPlayer.start();
			...
			stopService(getApplicationContext(), getClass());
		}
	}
